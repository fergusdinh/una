...
import { SubscriptionPlugin } from '../../src';
import { SubscriptionService } from '../../src/api/subscription.service';
...
describe('Test Report plugin', function () {
  ...
  it('Should start successfully', async () => {
    nock('https://testing')
      .persist()
      .get('/api/webhooks/public_key')
      .reply(200, mockData.lagoWebhookTokenResponse);
    const key = await server.app.get(SubscriptionService).webhooksPublicKey();
    await expect(key).toBe(true);
  });
